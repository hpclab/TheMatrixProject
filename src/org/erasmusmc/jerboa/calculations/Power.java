...
public class Power {
	public static void main(String[] args){
		System.out.println(exposureNeeded(0.05, 0.8, 1.5, 0.000056627994));
	}
	
	/**
	 * Calculates the amount of exposure needed to pick up a signal
	 * 
	 * @param alpha	Usually 0.05
	 * @param power Usually 0.8
	 * @param rr	Relative risk
	 * @param ir	Incidence rate (events/year)
	 * @return Exposure (in days) needed to pick up a signal
	 */
	public static long exposureNeeded(double alpha, double power, double rr, double ir){
		double lambda0 = ir;
		double delta = (rr-1)*lambda0;
		double lambda1 = lambda0 + delta;
		double z1minusAlpha = getInvCDF(1-alpha,true);
		double beta = 1-power;
		double z1minusBeta = getInvCDF(1-beta,true);
		double num = Math.pow(z1minusAlpha*Math.sqrt(lambda0)+z1minusBeta*Math.sqrt(lambda1),2);
		double years = num/Math.pow(delta, 2);
